  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto pb-8 space-y-6" style={{ paddingTop: '20px', paddingLeft: '28px', paddingRight: '28px' }}>
        ...
        <div style={{ ...cardSt, borderRadius: '16px' }}>
          {/* Kart başlık şeridi */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', padding: '14px 28.3px', background: 'var(--surface3)', borderBottom: '1px solid var(--border)', borderRadius: '16px 16px 0 0' }}>
            ...
            <div ref={scenarioRef} style={{ position: 'relative' }}>
              <button
                ...
                style={{
                  background: scenarioOpen ? 'var(--acc-bg)' : 'var(--surface2)',
                  border: `1px solid ${scenarioOpen ? 'var(--acc-bd)' : 'var(--border)'}`,
                  color: scenarioOpen ? 'var(--acc-tx)' : 'var(--tx2)',
                  cursor: 'pointer',
                }}>
                <span>⚡</span>
                <span>Strateji Seç</span>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
                  className="w-3 h-3"
                  style={{ transform: scenarioOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>
              ...
              {scenarioOpen && (
                <div style={{
                  ...
                  {SCENARIOS.map((s, idx) => (
                    <button key={s.id}
                      onClick={() => { setCriteria(s.criteria); setSelectedScenario(s); setScenarioOpen(false); }}
                      className="w-full text-left flex items-start gap-3 px-4 py-3 transition-colors"
                      style={{
                        background: selectedScenario?.id === s.id ? 'var(--acc-bg)' : 'transparent',
                        borderBottom: idx < SCENARIOS.length - 1 ? '1px solid var(--border)' : 'none',
                        borderRadius: idx === 0 ? '14px 14px 0 0' : idx === SCENARIOS.length - 1 ? '0 0 14px 14px' : '0',
                        cursor: 'pointer',
                      }}
                      onMouseEnter={e => { if (selectedScenario?.id !== s.id) (e.currentTarget as HTMLElement).style.background = 'var(--surface2)'; }}
                      onMouseLeave={e => { if (selectedScenario?.id !== s.id) (e.currentTarget as HTMLElement).style.background = 'transparent'; }}>
                      <span style={{ fontSize: '20px', lineHeight: 1, flexShrink: 0, marginTop: '1px' }}>{s.emoji}</span>
                      <div style={{ minWidth: 0 }}>
                        <div className="font-semibold text-[13px]" style={{ color: selectedScenario?.id === s.id ? 'var(--acc-tx)' : 'var(--tx1)' }}>
                          {s.name}
                        </div>
                        <div className="text-[11px] mt-0.5" style={{ color: 'var(--tx3)' }}>{s.tagline}</div>
                      </div>
                    </button>
                  ))}
                </div>
          ...
          {selectedScenario && (
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '12px 20px', background: 'var(--acc-bg)', borderBottom: '1px solid var(--acc-bd)' }}>
              <span style={{ fontSize: '18px', lineHeight: 1, flexShrink: 0, marginTop: '1px' }}>{selectedScenario.emoji}</span>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--acc-tx)' }}>
                  {selectedScenario.name} · {selectedScenario.tagline}
                </div>
                <div style={{ fontSize: '12px', color: 'var(--tx2)', marginTop: '3px', lineHeight: 1.5 }}>
                  {selectedScenario.description}
                </div>
              </div>
              <button
                onClick={() => setSelectedScenario(null)}
                style={{ flexShrink: 0, background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--tx3)', padding: '2px', marginTop: '1px' }}
