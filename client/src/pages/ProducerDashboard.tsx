...
function Badge({ active }: { active: boolean }) {
  return <Pill variant={active ? 'ok' : 'neutral'}>{active ? 'Aktif' : 'Pasif'}</Pill>;
}
  ...
  return (
    <div className="flex items-center gap-4 px-5 py-4 transition-colors hover:bg-[var(--surface2)]"
      ...
      <div className="flex items-center gap-2 shrink-0">
        <Button variant="secondary" size="sm" onClick={onSelect}>Yönet</Button>
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-lg p-5 space-y-4"
      ...
      <div className="flex gap-3">
        ...
        <Button type="submit" variant="primary" fullWidth disabled={loading} loading={loading}>Ekle</Button>
      </div>
    </form>
  );
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <Button variant="secondary" onClick={handleImpersonate} disabled={impLoad} loading={impLoad}
          icon={
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
      ...
      <div className="rounded-lg overflow-hidden" style={{ border: '1px solid var(--border)' }}>
        ...
        {showForm && (
          <form onSubmit={handleAddUser} className="px-5 py-4 space-y-3"
            ...
            <div className="flex gap-2">
              <Button type="button" variant="secondary" fullWidth onClick={() => setShowForm(false)}>İptal</Button>
              <Button type="submit" variant="primary" fullWidth disabled={formLoad} loading={formLoad}>Ekle</Button>
            </div>
          </form>
        )}
        ...
        {loading ? (
          ...
          <div key={u.id} className="flex items-center gap-4 px-5 py-3"
            ...
            <Pill variant={u.role === 'super_admin' ? 'accent' : 'neutral'}>
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      ...
      <div className="flex items-center justify-between px-8 py-4 border-b" style={{ borderColor: 'var(--border)', background: 'var(--surface)' }}>
        <div className="flex items-center gap-3">
          ...
          <span className="font-serif text-sm font-medium" style={{ color: 'var(--tx1)' }}>Rank<em>ify</em></span>
          <Pill variant="accent">Üretici Paneli</Pill>
        </div>
      ...
      <div className="px-8">
        <PageHeader
          eyebrow="Çok Markalı Yönetim"
          title="Markalar"
          action={!selected && !showForm && (
      ...
      <KpiStrip items={[
        { label: 'Toplam Marka', value: tenants.length || null },
        { label: 'Aktif Marka', value: tenants.length ? tenants.filter(t => t.isActive).length : null, tone: 'var(--acc2-tx)' },
        { label: 'Toplam Kullanıcı', value: tenants.length ? tenants.reduce((s, t) => s + (t.userCount ?? 0), 0) : null },
      ]} />
      ...
      <div className="max-w-4xl mx-auto py-6 px-8 space-y-6">
        ...
        {selected ? (
          ...
          <div className="rounded-lg py-16 text-center" style={{ border: '1px dashed var(--border)' }}>
