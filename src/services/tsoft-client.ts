 ...
 *  - Explicit ListNo (0 < n < 999999) olan ürünler önce, ListNo artan sırada
 *  - Geri kalanlar (ListNo=0 veya 999999) arkada, registrationDate azalan (yeniden eskiye)
 *    T-Soft kategori sayfaları sırasız ürünleri "en yeni önce" gösterir.
 */
function sortByTsoftOrder(products: TSoftProduct[], logPrefix?: string): void {
  const SENTINEL = 999999;
  const hasMeaningful = products.some(p => p.sortOrder > 0 && p.sortOrder < SENTINEL);

  products.sort((a, b) => {
    const aExplicit = a.sortOrder > 0 && a.sortOrder < SENTINEL;
    const bExplicit = b.sortOrder > 0 && b.sortOrder < SENTINEL;

    if (aExplicit && bExplicit) return a.sortOrder - b.sortOrder;
    if (aExplicit) return -1;
    if (bExplicit) return 1;
    // İkisi de sırasız → registrationDate azalan (yeniden eskiye)
  ...
  if (logPrefix) {
    logger.info(`${logPrefix} toplam=${products.length} anlamlı-listno=${hasMeaningful ? 'var' : 'yok'}`);
  }
}
...
export class TSoftClient {
  ...
  async getCategoryProductsSorted(categoryId: string): Promise<TSoftProduct[]> {
    ...
    sortByTsoftOrder(results, `[getCategoryProductsSorted] kategori=${categoryId}`);
    return results;
  }
  ...
  async getCategoryProductsFull(categoryId: string): Promise<TSoftProduct[]> {
    ...
    while (true) {
      ...
      if (start === 0 && batch.length > 0) {
        const first = batch[0];
        logger.info(`[getCategoryProductsFull] StatViews=${first.StatViews ?? first.statViews ?? 'YOK'} CountTotalSales=${first.CountTotalSales ?? first.countTotalSales ?? 'YOK'} — tüm anahtarlar: ${Object.keys(first).join(', ')}`);
      }
      results.push(...batch.map(p => this.mapProduct(p)));
    ...
    sortByTsoftOrder(results, `[getCategoryProductsFull] kategori=${categoryId}`);
  ...
  async getProductStats(days: number): Promise<TSoftProductStats[]> {
    ...
    // Bu endpoint bazı T-Soft hesaplarında kapalıdır ("Controller is not allowed!").
    // Kapalı olduğunda tsoftViews skoru 0 kalır; T-Soft panelinizde bu raporu açmanız gerekir.
    try {
      let start = 0;
      const limit = 200;
      while (true) {
        ...
        if (d?.success === false) {
          ...
          logger.error(`[getProductStats] report/getProductVisitReport bu hesapta kapalı: "${msg}" — tsoftViews skoru 0 olacak. T-Soft panelinizde "Ürün Ziyaret Raporu" iznini kontrol edin.`);
          break;
        }
        const rows = this.extractRows(raw);
        for (const r of rows) {
